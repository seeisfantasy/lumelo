...
## `docs/archive/V1_Technical_Architecture_Proposal.md` (1/2)
...
## `docs/archive/V1_Technical_Architecture_Proposal.md` (2/2)
...
## `examples/sysctl/90-t4-audio.conf.sample`

- bytes: 335
- segment: 1/1

~~~text
# Conservative first-pass sysctl baseline for an audio-focused RK3399 system.
# Start here, measure, then tighten or relax.

vm.swappiness = 10
vm.vfs_cache_pressure = 50
vm.min_free_kbytes = 65536

# Cap dirty data so flash writeback does not build up into large bursts.
vm.dirty_background_bytes = 16777216
vm.dirty_bytes = 67108864
~~~

## `examples/systemd/background-service.override.conf.sample`
...
~~~text
[Service]
# Template for scan, web, discovery and NAS-adjacent services.
CPUAffinity=0 1 2 3
Nice=5
...
## `examples/systemd/mpd.override.conf.sample`
...
~~~text
[Service]
# Keep playback work on the big cores once the CPU map is verified.
CPUAffinity=4 5
Nice=-11
...
# Enable only after driver and long-run stability validation.
...
Place media, config, and protocol fixtures for local development and tests here.
~~~
