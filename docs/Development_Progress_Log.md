...
### 8.14 2026-04-13：清理主文档口径，并生成给外部 AI 的静态 review 文档包

这轮顺手把文档做了一次收口，目标是减少过期口径，并让外部 AI 可以在没有仓库挂载权限时也做静态审查。

主文档清理：

- `AI_Handoff_Memory.md`
  - 删除了已经过期的“先上 `v16`、先做蓝牙/Wi‑Fi bring-up”接手说明
  - 改成当前阶段的接手顺序：
    - 优先补真 TF / USB 闭环
    - 补整机重启状态回归
    - 再决定是否在索引层过滤坏文件
...
- [scripts/build-ai-review-docs.py](/Volumes/SeeDisk/Codex/Lumelo/scripts/build-ai-review-docs.py)
...
- 自动约束：
  - 单个 review 文档 `< 60 KB`
  - 自动跳过 binary / 非 UTF-8 文件
  - 避免把 `docs/review/` 自己再次递归打包

本地验证：

- `python3 -m py_compile scripts/build-ai-review-docs.py`
- `python3 scripts/build-ai-review-docs.py`

当前导出结果：

- 入口：
  - [docs/review/README.md](/Volumes/SeeDisk/Codex/Lumelo/docs/review/README.md)
- 索引：
  - [docs/review/AI_Review_File_Index.md](/Volumes/SeeDisk/Codex/Lumelo/docs/review/AI_Review_File_Index.md)
- 分卷：
  - `AI_Review_Part_01.md` 到 `AI_Review_Part_24.md`
- 所有生成文件都已控制在 `60 KB` 以内

阶段结论：

- 当前 `docs/` 顶层主文档已经再次收口
- 外部 AI 现在可以只拿 `docs/review/` 做静态代码审查，而不必直接挂载仓库
