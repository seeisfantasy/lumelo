...
expect_file /etc/systemd/system/lumelo-mode-manager.service "mode manager unit"
expect_executable /usr/libexec/lumelo/mode-manager "mode manager helper"
expect_file /etc/tmpfiles.d/lumelo.conf "Lumelo tmpfiles policy"
expect_text /etc/tmpfiles.d/lumelo.conf "d /run/lumelo 0750" "runtime dir tmpfiles mode"
expect_text /etc/tmpfiles.d/lumelo.conf "d /var/lib/lumelo 0750" "state dir tmpfiles mode"
expect_file /etc/systemd/system/local-mode.target "local-mode target"
if grep -F "media-indexd.service" "${ROOTFS_MOUNT}/etc/systemd/system/local-mode.target" >/dev/null; then
  fail "local-mode target should not directly want media-indexd.service"
else
  pass "local-mode target keeps media-indexd as on-demand worker"
fi
expect_file /etc/systemd/system/bridge-mode.target "bridge-mode target"
expect_text /etc/systemd/system/bridge-mode.target "controld.service" "bridge-mode target controld dependency"
expect_file /etc/systemd/system/playbackd.service "playbackd unit"
expect_text /etc/systemd/system/playbackd.service "RuntimeDirectoryMode=0750" "playbackd runtime dir mode"
expect_file /etc/systemd/system/sessiond.service "sessiond unit"
expect_text /etc/systemd/system/sessiond.service "RuntimeDirectoryMode=0750" "sessiond runtime dir mode"
expect_file /etc/systemd/system/media-indexd.service "media-indexd unit"
expect_text /etc/systemd/system/media-indexd.service "RuntimeDirectoryMode=0750" "media-indexd runtime dir mode"
expect_file /etc/systemd/system/controld.service "controld unit"
expect_text /etc/systemd/system/controld.service "CONTROLD_SSH_APPLY_COMMAND=/usr/libexec/lumelo/ssh-apply" "controld SSH apply helper"
expect_text /etc/systemd/system/controld.service "RuntimeDirectoryMode=0750" "controld runtime dir mode"
expect_file /etc/systemd/system/auth-recovery.service "auth recovery unit"
expect_executable /usr/libexec/lumelo/auth-recovery "auth recovery helper"
expect_executable /usr/libexec/lumelo/ssh-apply "SSH apply helper"
...
expect_symlink /etc/systemd/system/multi-user.target.wants/lumelo-mode-manager.service "mode manager enablement"
